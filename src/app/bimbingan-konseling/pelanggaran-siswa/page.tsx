...
import { dummyPelanggaranSiswa, dummyJenisPelanggaran, dummyTindakLanjut } from '@/data/bk';
import { dummyStudents } from '@/data/students';
import { PelanggaranSiswa } from '@/types';

function PelanggaranSiswaContent() {
  const { user } = useAuth();
  const router = useRouter();
  const [search, setSearch] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState<string | null>(null);
  const [data, setData] = useState<PelanggaranSiswa[]>([...dummyPelanggaranSiswa]);
  ...
  const filtered = data.filter((item) => {
    ...
    const jp = dummyJenisPelanggaran.find((j) => j.id === item.jenisPelanggaranId);
    const tl = dummyTindakLanjut.find((t) => t.id === item.tindakLanjutId);
    const searchStr = `${student?.namaLengkap || ''} ${jp?.nama || ''} ${tl?.nama || ''} ${item.deskripsi} ${item.tanggal}`.toLowerCase();
  ...
  const openEdit = (item: PelanggaranSiswa) => {
    setEditId(item.id);
    setFormData({
      tanggal: item.tanggal,
      siswaId: item.siswaId,
      jenisPelanggaranId: item.jenisPelanggaranId,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editId) {
      ...
      setData((prev) => [...prev, { id: `ps${Date.now()}`, ...formData }]);
    }
    setShowForm(false);
  };

  const violationTindakLanjut = dummyTindakLanjut.filter((t) => t.kategori === 'Pelanggaran');

  return (
    <MainLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Data Pelanggaran Siswa ({filtered.length})</h1>
          <p className="text-gray-500 mt-1">Catatan pelanggaran siswa yang diinput oleh admin</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-gray-600">Jenis Pelanggaran</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-red-600 text-xs">{jp?.nama || '-'}</span>
                        </td>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
                <h3 className="text-lg font-semibold text-gray-800">{editId ? 'Edit' : 'Tambah'} Pelanggaran Siswa</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={formData.jenisPelanggaranId}
                    onChange={(e) => setFormData({ ...formData, jenisPelanggaranId: e.target.value })}
                    required
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                  >
                    <option value="">Pilih Jenis Pelanggaran</option>
                    {dummyJenisPelanggaran.map((jp) => (
                      <option key={jp.id} value={jp.id}>{jp.nama} (Skor: {jp.skor}, {jp.level})</option>
                    ))}
                  </select>
                </div>
                <div>
...
export default function PelanggaranSiswaPage() {
  return <PelanggaranSiswaContent />;
}
